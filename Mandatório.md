...
**Checkpoint atual:** release 0.43 construída a partir da 0.42 (`d9936dc`) e publicada com três módulos novos: `app.remote43.js`, `xtream-ui-v43.js` e `live-log-overlay-v43.js`. O live voltou a tolerar buffering como a referência 0.31/0.32 e só entrega fallback externo após falha final apoiada por modo + relógio; o overlay de log agora fecha de forma idempotente, aborta XHR e invalida resposta tardia ao sair do live. Os 21 módulos do manifesto 0.43 conferiram em bytes/SHA-256 no remoto e os checks focados passaram por inspeção/parse do conteúdo remoto; `tests/remote43.test.cjs` foi publicado, mas Node/jsdom/Playwright e streaming físico ainda não foram executados neste ambiente. A ativação usa `version=0.43` como último marcador; na TV, validar canal Vivo, buffering transitório, fallback terminal e live→VOD/episódio com log aberto. Evidência completa em `REMOTE_43_VALIDATION.md`.
