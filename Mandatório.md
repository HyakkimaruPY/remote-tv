...
**Checkpoint atual:** release 0.42 preparada sobre `1d61f76`; navegação e paginação live preservam o contêiner, a fila de logos não reinicia a imagem ativa e o seek de filmes/episódios só altera o player principal após OK. 50 cenários jsdom/ES5 e 6 do YouTube 3 passaram; os 21 módulos conferem em bytes/SHA-256. O teste Playwright continua não executado por ausência de Chromium neste ambiente. Publicação em duas etapas, marcador `version` por último. Evidência e limites em `REMOTE_42_VALIDATION.md`; controle físico, decoder/preview e streaming real ainda exigem confirmação na TV.
