...
**Checkpoint atual:** release 0.41 preparada sobre `3d8c313`; o boot remove o `rawm3u.remote5.js` legado e o YouTube 3 do caminho crítico, mantém o catálogo Xtream lazy e adiciona recuperação live somente após falha terminal confirmada. 48 cenários jsdom/ES5 e 6 do YouTube 3 passaram; os 21 módulos conferem em bytes/SHA-256. O teste visual passou pela montagem lógica do manifesto, mas o screenshot Playwright ficou não executado por ausência de Chromium neste ambiente. Publicação continua em duas etapas, marcador `version` por último. Evidência e limites em `REMOTE_41_VALIDATION.md`; controle físico, helper nativo e streaming real ainda exigem confirmação na TV.
