...
var oc=X.cats,oi=X.items,ov=X.vod,os=X.series,busy=0,q=[],epoch31=0,cancel31=X.cancel;
function flush(e){var a=q.slice(0),i;q=[];busy=0;for(i=0;i<a.length;i++)try{a[i](e)}catch(y){X.log('AUTH30','callback erro · '+y)}}
function ensure(cb,why){var ticket31=epoch31,prior=X.state&&X.state.c?{d:X.state.c.d,u:X.state.c.u,p:X.state.c.p}:null,transient;
  ...
  X.auth(function(e,meta){if(ticket31!=epoch31)return;
...
X.cancel=function(){epoch31++;busy=0;q=[];cancel31()};X.ensureAuth=ensure;
...
R.authGuard='0.31';X.log('BOOT','Auth guard v0.30 · revalida CONFIG/DNS · preserva sessão válida em falha transitória');
}(this));
